...
def make_default_form(description: str = "", *, label: str = "默认造型") -> dict[str, Any]:
    return {
        "label": label,
        "description": description or "",
        "storyboard_ref_slot": DEFAULT_STORYBOARD_REF_SLOT,
        "input_refs": [],
        "refs": make_default_refs(),
    }


def normalize_form(form: dict[str, Any] | None, *, description: str = "", label: str = "默认造型") -> dict[str, Any]:
    ...
    normalized = {
        ...
        "storyboard_ref_slot": data.get("storyboard_ref_slot") or DEFAULT_STORYBOARD_REF_SLOT,
    ...
    if normalized["storyboard_ref_slot"] not in CHARACTER_REF_SLOTS:
        normalized["storyboard_ref_slot"] = DEFAULT_STORYBOARD_REF_SLOT
    ...
    if forms_src:
        for form_id, form in forms_src.items():
            try:
                ...
                continue
            forms[normalized_id] = normalize_form(form)
    if not forms:
        forms[DEFAULT_FORM_ID] = make_default_form(description)
...
def ensure_character_forms(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(entry.get("forms"), dict) or not entry["forms"]:
        entry["forms"] = {DEFAULT_FORM_ID: make_default_form(str(entry.get("description") or ""))}
...
def set_ref_path(entry: dict[str, Any], form_id: str, slot: str, path: str) -> None:
    ...
    if form_id not in entry["forms"]:
        entry["forms"][form_id] = make_default_form()

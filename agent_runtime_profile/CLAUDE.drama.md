...
### 音频规范
- **背景音乐自动禁止**：在视频 prompt 末尾统一追加"禁止出现：背景音乐、血迹、文字字幕、水印"
...
主 Agent（编排层 — 极轻量）
  ...
  ├─ dispatch → adapt-reference-video-episode 参考模式单集短剧改编规划
...
| generate-assets | `/generate-assets` | 统一资产生成：角色用 character_refs，场景/道具用 scene/prop |
...
4. **单集预处理** → 按 `effective_mode` 选：
   - reference_video → 先 `adapt-reference-video-episode` 生成 `step0_episode_adaptation.md`，再 `split-reference-video-units` 生成 `step1_reference_units.md`
   - narration → `split-narration-segments`
   - drama → `normalize-drama-script`
5. **JSON 剧本生成** → dispatch `create-episode-script` subagent
6. **资产设计（角色形态/scene/prop 并行）** → dispatch `generate-assets` subagent
...
- `characters`：角色完整定义（description、voice_style、default_form、forms；每个形态含 full_body / three_view / input_refs）
